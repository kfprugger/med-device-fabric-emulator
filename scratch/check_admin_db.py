...
def main():
    ...
    # 1. List all tables in admin schema
    try:
        cursor.execute("""
        ...
        print(f"Found {len(tables)} tables in Admin database:")
        ...
        # 2. Query contents of key configuration and status tables
        for s_name, t_name in tables:
            if "status" in t_name.lower() or "history" in t_name.lower() or "config" in t_name.lower() or "movement" in t_name.lower() or "process" in t_name.lower() or "ingest" in t_name.lower():
                print(f"\n--- Top 10 records from {s_name}.{t_name} ---")
                try:
                    cursor.execute(f"SELECT TOP 10 * FROM [{s_name}].[{t_name}]")
